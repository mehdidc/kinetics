...
def download_clip(video_identifier, output_filename,
    ...
    print('Download {}'.format(video_identifier))
    status = False
...
def main(input_csv, output_dir,
    ...
    if labels:
        qs = [dataset['label-name'] == label for label in labels]
        f = reduce(lambda a, b: a | b, qs)
        dataset = dataset[f]
        print('Dataset size after filtering labels: {}'.format(len(dataset)))
    if nb_examples:
        np.random.seed(seed)
        dataset = dataset.sample(n=nb_examples)
        print('Dataset size after filtering nb examples: {}'.format(len(dataset)))
